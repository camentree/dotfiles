...
4. Style. A fresh sub-agent gets the diff, every `## Code style checks` section from `~/.claude/CLAUDE.md`, the project's CLAUDE.md, and its CLAUDE.local.md, and the files beside each changed one. It returns findings as `file:line`, the rule, and the fix. Anything it reports gets fixed.
